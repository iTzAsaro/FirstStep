...
export async function createDb(env: Env): Promise<Db> {
  ...
  try {
    ...
    await sql.unsafe(`CREATE INDEX IF NOT EXISTS idx_talent_profiles_company_user_id ON talent_profiles (company_user_id)`, []);
  } catch { }
  ...
  try {
    ...
    await sql.unsafe(`CREATE INDEX IF NOT EXISTS idx_cvs_user_id ON cvs (user_id)`, []);
  } catch { }
  ...
  try {
    ...
    await sql.unsafe(`CREATE INDEX IF NOT EXISTS idx_ai_sessions_user_kind ON ai_sessions (user_id, kind)`, []);
  } catch { }
  ...
  try {
    ...
    await sql.unsafe(`CREATE INDEX IF NOT EXISTS idx_ai_messages_session_id ON ai_messages (session_id)`, []);
  } catch { }
  ...
  try {
    await sql.unsafe(`DROP TRIGGER IF EXISTS trg_users_set_updated_at ON users`, []);
    await sql.unsafe(`CREATE TRIGGER trg_users_set_updated_at BEFORE UPDATE ON users FOR EACH ROW EXECUTE FUNCTION set_updated_at()`, []);
    await sql.unsafe(`DROP TRIGGER IF EXISTS trg_talent_profiles_set_updated_at ON talent_profiles`, []);
    await sql.unsafe(
      `CREATE TRIGGER trg_talent_profiles_set_updated_at BEFORE UPDATE ON talent_profiles FOR EACH ROW EXECUTE FUNCTION set_updated_at()`,
      [],
    );
    await sql.unsafe(`DROP TRIGGER IF EXISTS trg_company_profiles_set_updated_at ON company_profiles`, []);
    await sql.unsafe(
      `CREATE TRIGGER trg_company_profiles_set_updated_at BEFORE UPDATE ON company_profiles FOR EACH ROW EXECUTE FUNCTION set_updated_at()`,
      [],
    );
    await sql.unsafe(`DROP TRIGGER IF EXISTS trg_cvs_set_updated_at ON cvs`, []);
    await sql.unsafe(`CREATE TRIGGER trg_cvs_set_updated_at BEFORE UPDATE ON cvs FOR EACH ROW EXECUTE FUNCTION set_updated_at()`, []);
    await sql.unsafe(`DROP TRIGGER IF EXISTS trg_ai_sessions_set_updated_at ON ai_sessions`, []);
    await sql.unsafe(
      `CREATE TRIGGER trg_ai_sessions_set_updated_at BEFORE UPDATE ON ai_sessions FOR EACH ROW EXECUTE FUNCTION set_updated_at()`,
      [],
    );
  } catch { }
  ...
  try {
    ...
    await sql.unsafe(`ALTER TABLE talent_profiles ADD COLUMN IF NOT EXISTS company_user_id BIGINT`, []);
    await sql.unsafe(`ALTER TABLE talent_profiles DROP CONSTRAINT IF EXISTS fk_talent_profiles_company`, []);
    await sql.unsafe(
      `ALTER TABLE talent_profiles
       ADD CONSTRAINT fk_talent_profiles_company
  ...
  try {
    ...
    await sql.unsafe(`ALTER TABLE IF EXISTS jobs DROP CONSTRAINT IF EXISTS chk_jobs_salary_range`, []);
    await sql.unsafe(
      `ALTER TABLE IF EXISTS jobs
